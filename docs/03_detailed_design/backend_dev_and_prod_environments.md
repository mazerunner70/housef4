...
| `DYNAMODB_IMPORT_STAGING_TABLE_NAME` | Required when import **§8.7** staging is enabled — DynamoDB Local table or Terraform **`aws_dynamodb_table.import_staging`** | Set in Lambda env from Terraform when implemented | Same key/GSI layout as primary; name pattern **`imports-in-progress`** — see [`database/data_model.md`](./database/data_model.md) §8.5 and [`import_transaction_files.md`](./import_transaction_files.md) **§8.7**. Omit for in-place import only (**§8.6** fallback). |
