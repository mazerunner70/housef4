...
| --- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Ingress**                     | Multipart extraction, validation, size limits (`extractImportMultipart`).                                                                                                                                                   | `file` buffer + form fields (`negate_amounts`, **`currency`** when creating account, account selectors).                                                                                      |
| 2   | **Resolve account**             | Create new account (with client **`currency`**) or validate existing (`FinanceRepository`). Reject file currency hint ≠ account currency → **`409 currency_mismatch`**.                                                                                              | `account_id`, locked **`currency`** for this import batch.                                                                                                                     |
...
| 12  | **Derive aggregates**           | `refreshStoredDashboardMetrics` — recompute **every `METRICS#<ISO4217>`** row for currencies on the user's accounts (may be **async** if UI shows **last-updated** on metrics).                                                                                                                                 | Per-currency `METRICS#…` snapshots.                                                                                                                                     |
...
**Q5 — Multi-currency?**  
**A5** — Each **account** has one **immutable currency** (chosen at create). Imports must match that currency or return **`409 currency_mismatch`**. Dashboard metrics are **per currency** (`METRICS#<ISO4217>`); never sum across currencies. See [`money_representation.md`](./money_representation.md).

**Q6 — Metrics refresh?**  
**A6** — **Async OK** if dashboard shows **last-updated** / staleness per **`METRICS#<ccy>`** metadata. Refresh **all** account currencies after import and tag-rule application.
