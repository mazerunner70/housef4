...
# Describe or create one table; retries while DynamoDB Local is still starting.
ensure_dynamo_table() {
  local name="$1"
  local retry=0
  local resolved=0
  ...
  while (( retry < MAX_ATTEMPTS )); do
    ...
    if (( describe_code == 0 )); then
      echo "Table $name already exists at $ENDPOINT"
      resolved=1
      break
    fi

    if grep -q 'ResourceNotFoundException' <<<"$describe_out"; then
      echo "Creating table $name at $ENDPOINT ..."
      aws "${create_table_cli_args[@]}" --table-name "$name"

      aws dynamodb wait table-exists --table-name "$name" --endpoint-url "$ENDPOINT" --region "$REGION"
    ...
    echo "aws dynamodb describe-table failed for $name:" >&2
    echo "$describe_out" >&2
    exit 1
  done
