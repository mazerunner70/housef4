...
/** Major-unit dollars → minor units (scale 2). */
...
test('proposalRootIds skips proposals when roots never initiate', () => {
  const r = computeAutoTransferPairings(
    [
      { id: 'e1', account_id: 'a', date: utc(2024, 0, 1), canonicalAmount: money(M(-10)) },
      { id: 'e2', account_id: 'b', date: utc(2024, 0, 1), canonicalAmount: money(M(10)) },
    ],
    {
      windowDays: 4,
      amountCurrency: 'USD',
...
test('proposalRootIds pairs when new leg initiates with counterparty', () => {
  ...
  const r = computeAutoTransferPairings(
    [
      { id: 'new1', account_id: 'a', date: utc(2024, 0, 1), canonicalAmount: money(M(-10)) },
      { id: 'old2', account_id: 'b', date: utc(2024, 0, 1), canonicalAmount: money(M(10)) },
    ],
    {
      windowDays: 4,
      amountCurrency: 'USD',
...
test('pairs inverse amounts on different accounts with exact confidence', () => {
  let n = 0;
  const legs = [
    { id: 'out', account_id: 'acc_chk', date: utc(2024, 2, 10), canonicalAmount: money(M(-250)) },
    { id: 'in', account_id: 'acc_sv', date: utc(2024, 2, 10), canonicalAmount: money(M(250)) },
  ];
...
test('respects epsilonAmount for non-zero residual', () => {
  let n = 0;
  const legs = [
    { id: 'a', account_id: 'x', date: utc(2024, 1, 1), canonicalAmount: money(M(-100.01)) },
    { id: 'b', account_id: 'y', date: utc(2024, 1, 1), canonicalAmount: money(M(100)) },
  ];
  const r = computeAutoTransferPairings(legs, {
    windowDays: 4,
    amountCurrency: 'USD',
    epsilonAmount: money(1),
    createPairingId: () => `pid-${++n}`,
  });
...
test('does not pair same account', () => {
  const r = computeAutoTransferPairings(
    [
      { id: 'a', account_id: 'acc1', date: utc(2024, 0, 1), canonicalAmount: money(M(-10)) },
      { id: 'b', account_id: 'acc1', date: utc(2024, 0, 1), canonicalAmount: money(M(10)) },
    ],
    { windowDays: 4, amountCurrency: 'USD', epsilonAmount: money(0), createPairingId: () => 'n/a' },
  );
  assert.deepEqual(r.byLegId, {});
});
...
test('does not pair when |Δt_ms| exceeds W × 86_400_000', () => {
  const r = computeAutoTransferPairings(
    [
      { id: 'a', account_id: 'x', date: utc(2024, 0, 1), canonicalAmount: money(M(-50)) },
      { id: 'b', account_id: 'y', date: utc(2024, 0, 6), canonicalAmount: money(M(50)) },
    ],
    { windowDays: 4, amountCurrency: 'USD', epsilonAmount: money(0), createPairingId: () => 'n/a' },
  );
  assert.deepEqual(r.byLegId, {});
...
test('pairs when |Δt_ms| equals W × 86_400_000 (nominal boundary)', () => {
  ...
  const r = computeAutoTransferPairings(
    [
      { id: 'a', account_id: 'x', date: utc(2024, 0, 1), canonicalAmount: money(M(-50)) },
      { id: 'b', account_id: 'y', date: utc(2024, 0, 5), canonicalAmount: money(M(50)) },
    ],
    { windowDays: 4, amountCurrency: 'USD', epsilonAmount: money(0), createPairingId: () => `pid-${++n}` },
  );
...
test('currency mismatch blocks pairing when both present', () => {
  const r = computeAutoTransferPairings(
    [
      { id: 'a', account_id: 'x', date: utc(2024, 0, 1), canonicalAmount: money(M(-20)), currency: 'USD' },
      { id: 'b', account_id: 'y', date: utc(2024, 0, 1), canonicalAmount: money(M(20)), currency: 'EUR' },
    ],
    { windowDays: 4, amountCurrency: 'USD', epsilonAmount: money(0), createPairingId: () => 'n/a' },
  );
  assert.deepEqual(r.byLegId, {});
...
test('collision: two debits compete for one credit — first root in §4 order claims credit', () => {
  let n = 0;
  const legs = [
    { id: 't_low', account_id: 'chk', date: utc(2024, 4, 1), canonicalAmount: money(M(-100)) },
    { id: 't_high', account_id: 'chk', date: utc(2024, 4, 1), canonicalAmount: money(M(-100)) },
    { id: 'credit', account_id: 'sv', date: utc(2024, 4, 1), canonicalAmount: money(M(100)) },
  ];
  const r = computeAutoTransferPairings(legs, {
    windowDays: 4,
    amountCurrency: 'USD',
...
test('tie-break prefers smallest |Δt_ms| then lexicographic (account_id, id) on B', () => {
  let n = 0;
  const legs = [
    { id: 'out', account_id: 'chk', date: utc(2024, 6, 10), canonicalAmount: money(M(-75)) },
    { id: 'later_inflow', account_id: 'zz_sv', date: utc(2024, 6, 13), canonicalAmount: money(M(75)) },
    { id: 'earlier_inflow', account_id: 'aa_sv', date: utc(2024, 6, 11), canonicalAmount: money(M(75)) },
  ];
  const r = computeAutoTransferPairings(legs, {
    windowDays: 4,
    amountCurrency: 'USD',
...
test('equal residual and date distance: lexicographically smaller partner wins', () => {
  ...
  const legs = [
    { id: 'debit', account_id: 'chk', date: sameDay, canonicalAmount: money(M(-40)) },
    { id: 'in_z', account_id: 'z_sv', date: sameDay, canonicalAmount: money(M(40)) },
    { id: 'in_a', account_id: 'a_sv', date: sameDay, canonicalAmount: money(M(40)) },
  ];
  const r = computeAutoTransferPairings(legs, {
    windowDays: 4,
    amountCurrency: 'USD',
...
test('deterministic root order: repeated runs match', () => {
  ...
  const legs = [
    ...[
      { id: `d${++seq}a`, account_id: 'chk', date: utc(2025, 0, 1), canonicalAmount: money(M(-10)) },
      { id: `d${seq}b`, account_id: 'chk', date: utc(2025, 0, 1), canonicalAmount: money(M(-10)) },
      { id: `c${seq}`, account_id: 'sv', date: utc(2025, 0, 1), canonicalAmount: money(M(10)) },
    ],
    ...[
      { id: `d${++seq}a`, account_id: 'chk', date: utc(2025, 0, 1), canonicalAmount: money(M(-10)) },
      { id: `d${seq}b`, account_id: 'chk', date: utc(2025, 0, 1), canonicalAmount: money(M(-10)) },
...
test('sorted pools matches brute force on mixed counterparts and proposals', () => {
  let id = 0;
  const mk = (date, amountMajor, acc) => ({
    id: `leg_${++id}`,
    account_id: acc,
    date,
    canonicalAmount: money(M(amountMajor)),
  });
  const counterparts = [
