...
import { parseOfxLike } from './parseOfx';
...
export function parseImportBuffer(
...
): { rows: ParsedImportRow[]; format: ImportSourceFormatKey | 'unknown' } {
  ...
  if (detected === 'ofx') {
    return { rows: tryOfx(), format: 'ofx' };
  }
  if (detected === 'qfx') {
    return { rows: tryOfx(), format: 'qfx' };
  }
  if (detected === 'qif') {
